...
public class JDownload extends JPanel implements ProgressListener, Runnable {
    ...
    public TransferEvent getLastEvent() {
        return lastEvent;
    }
        ...
        public void run() {
            ...
            switch (te.getState()) {
                ...
                case TransferEvent.DECOMPRESSING:
                    JDownload.this.startingPos = te.getStartingPosition();
                    if (te.getState() == TransferEvent.TRANSFERING) {
                        StringBuffer sb = new StringBuffer();
                        sb.append("downloading");
                        //sb.append(dl.getURL().toString());
                        sb.append(" -> ");
                        sb.append(te.getFile().getAbsolutePath());
                        textPane.setText(sb.toString());
                        totalSize = dl.getContentLength();
                    } else {
                        textPane.setText("decompressing -> " + te.getFile().getAbsolutePath());
                ...
                default:
                    log.warn("unexpected event: " + te.getStateLabel());
            }
