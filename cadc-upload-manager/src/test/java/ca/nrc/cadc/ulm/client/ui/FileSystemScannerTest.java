        ...
        extends AbstractCADCVOSTest<FileSystemScanner>
{
    ...
    protected void initializeTestSubject() throws Exception
    {
        ...
        setTestSubject(new FileSystemScanner(null, null, null));
    }
    ...
    public void isSymLink() throws Exception
    {
        ...
        // a directory
        File file = FileLoader.fromClasspath("testDir");
        assertFalse("Directory is not a symlink", scanner.isSymLink(file));

        // a file
        file = FileLoader.fromClasspath("testFile");
        assertFalse("File is not a symlink", scanner.isSymLink(file));

        // resources/testDirSymlink is a symlink to resources/testDir
        // Unreliable test!
//        file = FileLoader.fromClasspath("testDirSymlink");
//        assertTrue("Should return true for a symlink", scanner.isSymLink(file));

        // parent directory as a symlink
        // Unreliable test!
//        file = FileLoader.fromClasspath("testDirSymlink/testDirFile");
//        assertTrue("Should return true for a symlink", scanner.isSymLink(file));

        // resource/testFileSymlink is a symlink to resources/testFile
        // Unreliable test!
//        file = FileLoader.fromClasspath("testFileSymlink");
//        assertTrue("Should return true for a symlink", scanner.isSymLink(file));
    }
    
    @Test
    public void queueContainerNode() throws Exception
    {
        ...
        File file = FileLoader.fromClasspath("testFile");
    ...
    @Test
    public void queueDataNode() throws Exception
    {
        CommandQueue queue = new CommandQueue(1, null);
        File sourceFile = new File("test");
        FileSystemScanner scanner =
                new FileSystemScanner(sourceFile, TEST_VOSURI, queue);
