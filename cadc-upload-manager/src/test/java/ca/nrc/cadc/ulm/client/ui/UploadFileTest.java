...
import ca.nrc.cadc.ulm.client.FileLoader;
...
public class UploadFileTest
{
    ...
    {
        Log4jInit.setLevel("ca.nrc.cadc.ulm", Level.INFO);
    }
    ...
    public void testNullNodeArgument() throws Exception
    {
        try
        {
            File testFile = FileLoader.fromClasspath("testFile");
        ...
        catch (IllegalArgumentException e)
        {
            assertTrue("wrong error",
                       e.getMessage().contains("dataNode cannot be null"));
        }
    }
    ...
    public void testNullFileArgument() throws Exception
    {
        ...
        {
            assertTrue("wrong error",
                       e.getMessage().contains("file cannot be null"));
        }
    }
    ...
    public void testFileActuallyADirectory() throws Exception
    {
        try
        {
            VOSURI uri = new VOSURI(ROOT_URI + "/dataNode");
            DataNode dataNode = new DataNode(uri);
            File testFile = FileLoader.fromClasspath("testDir");
        ...
        {
            assertTrue("wrong error",
                       e.getMessage().contains("not a file"));
        }
    }
    ...
    public void testFileNotReadable() throws Exception
    {
        ...
        {
            assertTrue("wrong error",
                       e.getMessage().contains("cannot read file"));
        }
        finally
        {
            if (testFile != null)
    ...
    public void testNewNode() throws Exception
    {
        ...
        EasyMock.expect(mockClient.getNode("/uploadFileTest/newDataNode",
                                           "limit=0&detail=min")).andThrow(
                new NodeNotFoundException(uri.getPath())).once();

        EasyMock.expect(mockClient.createNode(dataNode, false))
                .andReturn(dataNode).once();
    ...
    private void setupSuccessfulFileUpload(DataNode dataNode, File testFile,
            ...
            throws Exception
    {
        ...
        ClientTransfer mockClientTransfer = EasyMock
                .createMock(ClientTransfer.class);
        ...
        EasyMock.expectLastCall().once();
        EasyMock.expect(mockClientTransfer.getThrowable()).andReturn(null)
                .once();
        EasyMock.expect(mockClientTransfer.getPhase())
                .andReturn(ExecutionPhase.COMPLETED).once();
